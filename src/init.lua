...
do
	...
	function cache.size(x0: boolean)
		return 1
	end

	function cache.ser(x0: boolean?, x1: boolean?, x2: boolean?, x3: boolean?, x4: boolean?, x5: boolean?, x6: boolean?, x7: boolean?)
...
do
	...
	cache.size = buffer.len

	cache.ser = buffer.tostring

	cache.des = buffer.fromstring

	function cache.serarr()

	end

	function cache.desarr()

	end

	function Squash.buffer()
		return cache
	end
end
...
do
	...
	function bytes1.size(x0: number)
		return 1
	end
	...
	function bytes2.size(x0: number)
		return 2
	end
	...
	function bytes3.size(x0: number)
		return 3
	end
	...
	function bytes4.size(x0: number)
		return 4
	end
	...
	function bytes5.size(x0: number)
		return 5
	end
	...
	function bytes6.size(x0: number)
		return 6
	end
	...
	function bytes7.size(x0: number)
		return 7
	end
	...
	function bytes8.size(x0: number)
		return 8
	end
...
-- 	elseif bytes == 3 then
-- 		return string.char(math.floor(x) % 256, math.floor(x / 256) % 256, math.floor(x / 256 ^ 2) % 256)
-- 	elseif bytes == 5 then
-- 		return string.char(
-- 			math.floor(x) % 256,
-- 			math.floor(x / 256) % 256,
